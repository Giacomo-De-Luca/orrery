...
from . import config as gemma_config, gemma3_preprocessor, model as gemma_model, tokenizer
...
class Gemma3ForMultimodalLM(nn.Module):
    ...
    def __init__(
        ...
        self.text_token_embedder = gemma_model.Embedding(
    ...
    def forward(
        ...
        **kwargs,
        ...
        freqs_cis = {}
        freqs_cis[gemma_config.AttentionType.LOCAL_SLIDING] = self.local_freqs_cis.index_select(
            0, input_positions
        )
        freqs_cis[gemma_config.AttentionType.GLOBAL] = self.global_freqs_cis.index_select(
            0, input_positions
        )
        ...
        if image_patches is not None and self.config.vision_config is not None:
            ...
            flattened_input = image_patches.reshape(B * N, C, H, W).to(self.dtype)  # (B*N)xCxHxW
    ...
    def generate(
        self,
        prompts: Sequence[Sequence[str | Image.Image]],
        device: Any,
        output_len: int = 100,
        ...
        temperatures_tensor = (
            None if not temperature else torch.FloatTensor([temperature] * batch_size).to(device)
        )
        ...
        for i in range(total_seq_len - min_prompt_len):
            ...
            # Early stop: check if all sequences have produced a stop token
            if stop_at_eot and not curr_prompt_mask.any():
                stop_ids = {self.tokenizer.eos_id, self.tokenizer.eot_id}
                if all(output_token_ids[b, 0].item() in stop_ids for b in range(batch_size)):
                    break
    ...
    def generate_stream(
        self,
        prompt: Sequence[str | Image.Image],
        device: Any,
        output_len: int = 256,
        temperature: float | None = 1.0,
        top_p: float = 0.95,
        ...
        """Yield (token_index, token_id, is_done) for each generated token.

        Like generate() but streams tokens one at a time via a Python generator.
        Only supports a single prompt (batch_size=1).

        Args:
            prompt: A single prompt sequence (list of interleaved str/Image).
            device: Target device (mps/cuda).
            output_len: Maximum tokens to generate.
            temperature: Sampling temperature. None for greedy.
            top_p: Nucleus sampling threshold.
            top_k: Top-k sampling threshold.
            stop_at_eot: Stop on EOS/EOT tokens.
            cancel_event: If set, generation stops after the current token.

        Yields:
            (token_index, token_id, is_done) tuples. token_index is 0-based
            within the generated output. is_done is True on the last token.
        """
        prompts = [prompt]
        processing_result = gemma3_preprocessor.tokenize_raw_input(
            self.tokenizer, prompts, self.config, output_len, device
        ...
        batch_size = processing_result["batch_size"]
        assert batch_size == 1, "generate_stream only supports batch_size=1"
        user_input_token_ids = processing_result["user_input_token_ids"]
        image_batch = processing_result["image_batch"]
        min_prompt_len = processing_result["min_prompt_len"]
        total_seq_len = processing_result["max_seq_len"]
        ...
        token_ids_tensor = user_input_token_ids.to(device)
        input_token_ids_tensor[0, :min_prompt_len] = token_ids_tensor[0, :min_prompt_len]
        ...
        output_positions_tensor = torch.LongTensor([min_prompt_len - 1]).to(device)
        temperatures_tensor = (
            None if not temperature else torch.FloatTensor([temperature]).to(device)
        )
        ...
        for i in range(total_seq_len - min_prompt_len):
            ...
            # Only yield once past the prefill phase
            if not curr_prompt_mask.any():
                token_id = output_token_ids[0, 0].item()
                cancelled = cancel_event is not None and cancel_event.is_set()
                is_stop = (stop_at_eot and token_id in stop_ids) or cancelled
                is_last = is_stop or (i == total_seq_len - min_prompt_len - 1)
                yield (generated_index, token_id, is_last)
                generated_index += 1
                if is_stop:
                    return
