...
`orrery_backend_data` volume only when `/data/main.duckdb` is absent. Existing
...
|--------|------------|----------|
| `orrery_backend_data` | `/data` | DuckDB, ChromaDB, uploads, job state, SAE labels, SAE vectors |
| `orrery_hf_cache` | `/models/huggingface` | HuggingFace model and SAE weight cache |
...
| `ORRERY_SEED_DIR` | `/app/interpretability_backend/resources/seed` | Read-only seed snapshot in image |
| `ORRERY_DIRECTIONS_DIR` | `/app/interpretability_backend/resources/directions` | Small shipped steering direction presets |
