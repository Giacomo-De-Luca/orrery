...
proto.plot = function(sceneData, fullLayout, layout) {
    ...
    // PATCH: detect whether scene-wide dataScale changed since the previous call.
    // Positions in convertPlotlyOptions() are multiplied by scaleFactor, so if
    // dataScale shifts every trace must be re-converted; otherwise we can use
    // per-trace reference-equality to skip unchanged traces entirely.
    var _prevDataScale = scene._lastDataScale;
    var _sameDataScale = !!_prevDataScale &&
        _prevDataScale[0] === dataScale[0] &&
        _prevDataScale[1] === dataScale[1] &&
        _prevDataScale[2] === dataScale[2];
    scene._lastDataScale = dataScale;
    ...
    // Update traces
    var _skipLog = { skipped: 0, updated: 0 };
    for(i = 0; i < sceneData.length; ++i) {
        data = sceneData[i];
        ...
        if(trace) {
            if(trace.data.type === data.type) {
                // PATCH: fast-path — if the user's input trace object reference
                // is unchanged and dataScale is the same, skip the expensive
                // per-trace convertPlotlyOptions + GPU buffer re-upload. Relies
                // on callers passing memoized (never mutated in place) traces.
                if(_sameDataScale && data.type === 'scatter3d' && trace._lastInput === data._input) {
                    trace.data = data; // refresh fullInput ptr for hover lookups
                    _skipLog.skipped++;
                } else {
                    trace.update(data);
                    trace._lastInput = data._input;
                    _skipLog.updated++;
                }
            } else {
                trace.dispose();
    ...
    // PATCH: temporary debug log — remove after validation
    if(typeof console !== 'undefined' && console.debug) {
        console.debug('[gl3d patch] scene.plot traces:', _skipLog, 'sameDataScale:', _sameDataScale);
    }
